...
class Vector:
    def __init__(self, init_list):
        if isinstance(init_list, list):
            ...
            if isinstance(init_list, Vector):
                ...
                raise TypeError(
                    "list or Vector expected but not {}".format(type(init_list)))

    def __eq__(self, value):
        if not (isinstance(value, list) or isinstance(value, Vector)):
            raise TypeError(
                "list or Vector expected but not {}".format(type(value)))
        if isinstance(value, list):
            for el in value:
                if not (isinstance(el, int) or isinstance(el, float)):
                    raise TypeError(
                        "Float or int objects are supported, not {}".format(type(el)))
    ...
    def __add__(self, other):
        if not (isinstance(other, list) or isinstance(other, Vector)):
            raise TypeError(
                "list or Vector expected but not {}".format(type(other)))
